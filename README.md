...
xiaoqiao-android-codex 不依赖 OCR，也不是去截图识别终端。
...
如果你希望在局域网外用 Android / iOS / Web 客户端访问 xiaoqiao-android-codex，可以把 Agent 和 Web 客户端只绑定到本机回环地址，再通过 Tailscale Service 暴露一个 tailnet 内部 HTTPS 入口。这样不需要把 xiaoqiao-android-codex 直接暴露到公网，也不需要在手机上记端口。
...
安全提醒：当前 xiaoqiao-android-codex Agent 还没有内置登录和设备配对机制。远程访问时建议只使用 tailnet 内部的 Tailscale Service，并配合 Tailscale ACL 限制可访问设备；不要用 Funnel 或公网反向代理直接公开 xiaoqiao-android-codex。
...
1. 打开 `会话` 页面，查看当前真实会话。
2. 对历史会话点击“接管到 xiaoqiao-android-codex”，将其转为受控会话。
...
- `Claude Code` 会话会进一步区分：
  ...
  - `已接管`：已经由 xiaoqiao-android-codex 托管
